...
//                                jedisShardInfo.setPassword("a6616270");
//                                jedisShardInfos.add(jedisShardInfo);
//                            }
                            //

                            jedisPool = new JedisPool(config,address, port, 10000, "a6616270");
    ...
    public static String setObjectValue(String key, Object obj, int seconds) {
        ...
        Jedis client = getInstance();
        try {
            result = client.setex(key.getBytes(), (long)seconds, serialize(obj));
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
        } finally {
            if (client != null) {
...
//        xxlSsoRedisAddress = "redis://127.0.0.1:";
